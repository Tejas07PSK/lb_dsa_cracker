...
class Solution:
    def __pathMoreThanKHelper (self, root, path, graph, v, k):
        ...
        for child in range(v):
            if ((child not in self.visited) and (graph[root][child] != -inf)):
    ...
    def pathMoreThanK (self, v, e, k, a):
        graph, self.visited = [[-inf for i in range(v)] for j in range(v)], set()
        for i in range(0, len(a), 3):
            graph[a[i]][a[i+1]] = max(graph[a[i]][a[i+1]], a[i+2])
            graph[a[i+1]][a[i]] = max(graph[a[i+1]][a[i]], a[i+2])
